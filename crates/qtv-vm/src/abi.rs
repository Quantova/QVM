...
//! verify region and the scheme identifiers the verify and address opcodes dispatch on. These are the
//! machine's own sizes, taken from the post quantum primitives it links, so a compiler that targets
//! this machine reads its region layout and address derivation from one source of truth rather than
//! restating the constants and risking a drift the machine would then reject.
