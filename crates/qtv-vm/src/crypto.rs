...
/// scheme byte, so an address the machine derives equals the one the chain derives.
...
/// pointer. The input region is the encapsulation key followed by the 32 byte message. Writes the
/// shared secret followed by the ciphertext at the output pointer.
...
/// public key at its start exactly as the verify opcodes read it, register `b` holds the signature
/// scheme identifier, and register `c` holds the output pointer. The address is SHA3 256 of the one
/// scheme byte followed by the scheme's public key bytes, the identical preimage the chain hashes in
/// `qtv_account::address_for_key`, so the machine derives the same thirty two byte address the chain
/// stored as an owner. Scheme one slices an ML-DSA public key and scheme two an SLH-DSA public key;
/// any other scheme faults, and a region too short to hold the public key faults. The digest is
/// written raw at the output pointer and is never formatted.
