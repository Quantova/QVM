...
The QVM is Quantova's execution layer. It is a register machine that runs compiled containers, with the post quantum primitives wired in as first class instructions. It is not the Ethereum virtual machine and it is not a WebAssembly runtime. The instruction set, the container format, the gas model, and the crypto opcodes are all our own.
